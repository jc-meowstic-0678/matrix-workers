    ...
    try {
      ...
      sortedUsers.forEach(user => {
        ...
        tr.innerHTML = \`
          ...
          <td>
            <button class="btn-icon-sm" onclick='filterByUser(\${JSON.stringify(user.userId)})' title="Show user's reports">👁️</button>
            <button class="btn-icon-sm warning" onclick='blockUser(\${JSON.stringify(user.userId)})' title="Block user">🚫</button>
          </td>
        \`;
        tbody.appendChild(tr);
      });
