    ...
    try {
      ...
      results.tests.forEach((test) => {
        ...
        tr.innerHTML = \`
          ...
          <td>\${test.message || '-'}</td>
          <td><button class="btn-icon" onclick='showTestDetails(\${JSON.stringify(test.name)})'>ℹ️</button></td>
        \`;
    ...
    try {
      ...
      paginatedServers.forEach((server) => {
        ...
        tr.innerHTML = \`
          ...
          <td>
            <button class="btn-icon" onclick='testServerConnection(\${JSON.stringify(server.server_name)})' title="Test connection">🔌</button>
            <button class="btn-icon" onclick='viewServerDetails(\${JSON.stringify(server.server_name)})' title="View details">📋</button>
          </td>
        \`;
        tbody.appendChild(tr);
      });
