...
// - HTTP fallback for device notifications when WebSockets aren't available
//
...
// ============================================
// HTTP FALLBACK FUNCTIONS (Added for scalability)
// ============================================

/**
 * Check if a device has an active WebSocket connection
 * Used to decide between WebSocket delivery and push notification
 */
...
export async function sendDeviceNotification(
  ...
  try {
    // Get the device's pusher configuration
    const pusher = await env.DB.prepare(`
      SELECT pushkey, app_id, data FROM pushers
    ...
    if (!pusher) {
      console.log(`[push] No pusher found for ${userId}/${deviceId}`);
      return false; // No pusher configured for this device
    }

    const pusherData = JSON.parse(pusher.data);
    
    // Check if this is an iOS device for direct APNs
    ...
    if (useDirectAPNs && isIOSPusher) {
      // Use direct APNs via PushDurableObject
      return await sendDirectAPNs(env, pusher, pusherData, event, counts);
    }

    // Fall back to Sygnal HTTP gateway
    return await sendViaSygnal(env, pusher, pusherData, event, counts);

  } catch (error) {
    console.error(`[push] Failed to send device notification to ${userId}/${deviceId}:`, error);
    return false;
  }
}
...
async function sendViaSygnal(
  ...
  try {
    ...
    // Deep clone default_payload and populate APNs alert
    ...
    if (response.ok) {
      console.log('[push] Sygnal delivery successful');
    } else {
      console.error('[push] Sygnal delivery failed:', response.status, await response.text());
    }

    return response.ok;

  } catch (error) {
    console.error('[push] Sygnal delivery failed:', error);
    return false;
  }
}
 ...
 * Now uses the HTTP fallback system for scalability
 */
export async function sendPushNotification(
  ...
  // Get user's devices with pushers
  const devices = await db.prepare(`
    SELECT d.device_id 
    FROM devices d
    JOIN pushers p ON d.user_id = p.user_id AND d.device_id = p.device_id
    WHERE d.user_id = ?
  ...
  console.log('[push] PUSH_DEVICES_FOUND:', {
    correlationId,
    userId,
    deviceCount: devices.results.length,
  });

  // For each device, check WebSocket and fall back to push
  const results = await Promise.allSettled(
    devices.results.map(async (device) => {
      try {
        // Check if device has active WebSocket
        const hasWebSocket = await hasActiveWebSocket(env, userId, device.device_id);
        
        if (hasWebSocket) {
          console.log('[push] PUSH_VIA_WEBSOCKET:', {
            correlationId,
            userId,
            deviceId: device.device_id,
          });
          // WebSocket will handle delivery automatically
          return { delivered: 'websocket', deviceId: device.device_id };
        }

        // Fall back to push notification
        console.log('[push] PUSH_VIA_HTTP_FALLBACK:', {
          correlationId,
          userId,
          deviceId: device.device_id,
        });

        const success = await sendDeviceNotification(env, userId, device.device_id, event, counts);
        
        return { 
          delivered: success ? 'push' : 'failed', 
          deviceId: device.device_id 
        };
      } catch (error) {
        console.error('[push] Device notification failed:', { correlationId, userId, device: device.device_id, error });
        return { delivered: 'error', deviceId: device.device_id, error };
      }
    })
  );
  ...
  const summary = results.reduce((acc, result) => {
    if (result.status === 'fulfilled') {
      acc[result.value.delivered] = (acc[result.value.delivered] || 0) + 1;
    } else {
  ...
  console.log('[push] PUSH_COMPLETED:', {
    correlationId,
    userId,
    ...summary,
  });
...
export async function notifyRoomMembersOfMessage(
  ...
  const notifications = members.results.map(async (member) => {
    try {
      ...
      // Send push notification via the new system
      await sendPushNotification(db, env, member.user_id, {
        ...event,
        sender_display_name: senderDisplayName,
        room_name: roomName,
      }, { unread: unreadCount });
